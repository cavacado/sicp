...
(define (generate-seq-of-pairs n)
  (accumulate append
              nil
              (map (lambda (i)
                     (map (lambda (j)
...
(define (prime-sum-pairs-refactor n)
  (map make-pair-sum
       (filter prime-sum?
               (unique-pairs n))))
...
(define wave2 (beside eins (flip-vert eins)))
...
(define wave4-new (flipped-pairs eins))
...
(define (split proc1 proc2)
  (lambda (painter n)
    (if (= n 0)
        painter
        (let ((smaller ((split proc1 proc2) painter
                                            (- n 1))))
          (proc1 painter
